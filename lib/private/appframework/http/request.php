...
//        throw new\Exception("path:$path,name:$name,pathInfo:$pathInfo");
		if (!empty($path)) {
			if($path === $pathInfo || strpos($pathInfo, $path.'/') === 0) {
				$pathInfo = substr($pathInfo, strlen($path));
			} else {
				throw new \Exception("The requested uri($requestUri @path:$path @orignrequestUri:$orignUri) cannot be processed by the script '$scriptName')");
			}
		}
