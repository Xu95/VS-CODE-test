...
if (\OC_Util::runningOnWindows()) {
	...
	class Local extends \OC\Files\Storage\Common {
		...
		public function mkdir($path) {
		    // update by DingNan @19/04/04 修改创建文件夹的权限
		    $oldMask = umask(0);
			$result = @mkdir($this->getSourcePath($path), 0777, true);
			umask($oldMask);
			return $result;
		}
		...
		public function touch($path, $mtime = null) {
			...
			// update By DingNan @ 19/04/04 更改新建文件的权限
			if ($result)
			    $result = chmod($this->getSourcePath($path), 0777);
			if ($result) {
