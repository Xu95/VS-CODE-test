...
(function() {
	...
	FileList.prototype = { 
		 ...
		 */
		pageSize: function() {
			//Math.round()取小数点后一位、四舍五入；Math.ceil()取上整；Math.floor()取下整
		...
		initialize: function($el, options) {
			...
			//yosang 文件列表#app-content-files
			this.$el = $el;
			if (options.id) {
				this.id = options.id;
			}
			//scrollContainer装文件列表的容器
		...
		//	this.setSort('name', 'asc');
			...
			var breadcrumbOptions = {
				//绑定函数到指定对象上，以后每次调用该函数时，this都指向该对象
		...
		_onUrlChanged: function(e) {
			if (e && e.dir) {
				this.changeDirectory(e.dir, false, true, e.view);
			}
		},
		...
		setPageTitle: function(title){
			...
			window.document.title = "我的文档"; /*title + ' - ' + oc_defaults.title;*/
		...
		_createRow: function(fileData, options) {
			...
			console.log(icon)
			if(/\.txt/.test(icon)){
				icon='/owncloud/core/img/filetypes/text.svg'
			}
			if (this._allowSelection) {
				td.append(
					...
					'<span class="hidden-visually">' + t('files', '选择') + '</span>' +
					'</label>'
				);
			} else {
			...
			var modifiedColor2 = Math.round(((new Date()).getTime() - mtime )/1000/60/60/24 );
			...
			if (mtime > 0) {
				...
				var indexMonth = months.indexOf(str[0]) + 1;
				var indexSecond = str[3].split(':');

				if((str[4]== 'PM') && (indexSecond[0] !=12)){
					var timeHour = parseInt(indexSecond[0])+ 12;
					var newtime = timeHour + ":" + indexSecond[1];
				}
				else{
					var newtime =  indexSecond[0] + ":" +indexSecond[1];
				}
				var newformatted = str[2]+'/'+ indexMonth +'/' + str[1] + ' ' + newtime;
				//console.log(newformatted);
				//text = OC.Util.relativeModifiedDate(mtime);
				if(modifiedColor2 < 30){
					text2 = modifiedColor2 + "天以前";
				}
				else{
					let month = parseInt(modifiedColor2 / 30);
					let year = parseInt(month / 12)
					if(year >= 1){
						tex2 = year + "年以前"
					}
					else text2 = month + "个月以前";
				}
			} else {
				formatted = t('files', '无法确定日期');
				text2 = '?';
			}
		 ...
		 */
		changeDirectory: function(targetDir, changeUrl, force,targetView) {
			var self = this;
			var currentDir = this.getCurrentDirectory();
			//yosang
			// if(targetView && (targetView.slice(0,targetView.length-1) == 'filesgroup')){
			// 	targetDir = targetDir+(targetView.slice(targetView.length-1)) || '/';
			// }else{
			// 	targetDir = targetDir || '/';
			// }
			targetDir = targetDir || '/';
			...
			//window.ytargetDir =targetDir;
			this.reload().then(function(success){
				//将其他的隐藏了，检测当前是哪个文件夹
				//yosang
				if(targetDir == '/personal'){
					//要更改当前的window.FileList
					
					window.commonFileList = window.FileList;
					OCA.Search.files.setFileList(window.commonFileList);
				}else{
					window.commonFileList = window.GroupFileList;
					OCA.Search.files.setFileList(window.commonFileList);
				}
				//更改搜索框中的内容
				OC.Search.changesearchBox()

				if(targetDir == '/'){
					$("#app-content-files  #fileList  tr").filter(function(index){
						return $(this).data('file')+0 == $(this).data('file')
					}).hide();
				}else{
					//应该更改一下当前的路径
				}
				$("#app-content > #app-content-files #controls .crumb[data-dir='/']").hide();
				$("#app-content > #app-content-files #controls .crumb:eq(1) a").html("<img class='svg' src='/owncloud/core/img/places/home.svg' alt='家庭'>");
				$("#app-content > div[id^='app-content-filesgroup']").each(function(index){
					$("#controls .crumb[data-dir='/']",this).hide();
				})
				$("#app-content > div[id^='app-content-filesgroup']").each(function(index){
					$("#controls .crumb:eq(1) a",this).html("<img class='svg' src='/owncloud/core/img/places/home.svg' alt='家庭'>")
				})
				if (!success) {
		...
		reloadCallback: function(result) {
			...
			if (result.status === 403) {
				// Go home
				this.changeDirectory('/personal');
				OC.Notification.show(t('files', '此操作是禁止的'));
				return false;
			}
			...
			if (result.status === 500) {
				// Go home
				this.changeDirectory('/personal');
				OC.Notification.show(t('files', '此目录不可用，请检查日志或与管理员联系'));
				return false;
			}
		...
		remove: function(name, options){
			...
			//如果该文件元素属于选中状态，那么取消它的选中状态，并且更新缓存
						...
						if (result) {
							if (result.status === 'success') {
								...
								if (result.status === 'error' && result.data.message) {
									...
									OC.Notification.show(t('files', '移动文件时出错'));
								}
							...
							OC.dialogs.alert(t('files', '移动文件时出错'), t('files', '错误'));
						}
			...
			var checkInput = function () {
				...
				if (filename !== oldname) {
					...
					if (self.inList(filename)) {
						throw t('files', '{new_name} 文件已存在', {new_name: filename});
					}
				}
				return true;
			};
					...
					if (newName !== oldname) {
						...
						$.ajax({
							...
							success: function(result) {
								var fileInfo;
								if (!result || result.status === 'error') {
									OC.dialogs.alert(result.data.message, t('files', '无法重命名文件'));
					...
					function(result) {
						...
						if (result.status === 'success') {
							...
							if (result.status === 'error' && result.data.message) {
								...
								OC.Notification.show(t('files', '删除文件时出错.'));
							}
							// hide notification after 10 sec
							setTimeout(function() {
								OC.Notification.hide();
			...
			if (this._filter && this.fileSummary.summary.totalDirs + this.fileSummary.summary.totalFiles === 0) {
				...
				if ( $('#searchresults').length === 0 || $('#searchresults').hasClass('hidden') ) {
					this.$el.find('.nofilterresults').removeClass('hidden').
						find('p').text(t('files', "此文件夹没有与 '{filter}' 匹配的条目", {filter:this._filter},  null, {'escape': false}));
				}
			} else {
		...
		//yosang
		//这个只有在移动端呼出键盘的时候才会触发，所以在pc浏览器中调试的时候没有问题，在移动端中会调用setFileList重新设置filelist对象
		updateSearchwithoutblur:function() {
			// alert('updateSearchwithoutblur')
			if (OCA.Search.files) {
				OCA.Search.files.setFileList(window.commonFileList);
			}
			if (OC.Search) {
				OC.Search.clearwithoutblur();
			}
		},
		updateSearchfornav:function() {
			// alert('updateSearchwithoutblur')
			if (OCA.Search.files) {
				OCA.Search.files.setFileList(window.commonFileList);
			}
		...
		updateSelectionSummary: function() {
			...
			if (summary.totalFiles === 0 && summary.totalDirs === 0) {
				...
				this.$el.find('#headerName a.name>span:first').text(t('files','名称'));
				this.$el.find('#headerSize a>span:first').text(t('files','大小'));
				this.$el.find('#modified a>span:first').text(t('files','修改'));
				...
				if (summary.totalDirs > 0) {
					selection += n('files', '%n 文件夹', '%n 文件夹', summary.totalDirs);
					if (summary.totalFiles > 0) {
						selection += ' 和 ';
					}
				}
				if (summary.totalFiles > 0) {
					selection += n('files', '%n 文件', '%n 文件', summary.totalFiles);
				}
				this.$el.find('#headerName a.name>span:first').text(selection);
				this.$el.find('#modified a>span:first').text('');
				this.$el.find('table').addClass('multiselect');
				// this.$el.find('.delete-selected').toggleClass('hidden', !canDelete);
                //  this.$el.find('.delete-selected').toggleClass('hidden');
		...
		_showPermissionDeniedNotification: function() {
			var message = t('core', '你没有权限在这里上传或创建文件');
		...
		setupUploadEvents: function() {
			...
			fileUploadStart.on('fileuploadadd', function(e, data) {
				OC.Upload.log('filelist handle add', e, data);
				...
				if (data.context && data.context.data('type') === 'dir') {
					...
					var translatedText = n('files', '上传 %n 文件', '上传 %n 文件', currentUploads);
					if (currentUploads === 1) {
...
$(document).ready(function() {
	...
	//使用bind为window元素添加beforeunload事件
	...
	$(window).unload(function () {
		//规定被选元素要触发的事件，触发window的beforeunload事件
