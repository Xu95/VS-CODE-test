...
OC.Upload = {
	...
	_hideProgressBarGroup: function(targetobj) {
		console.log($(targetobj).parents("div[id^='app-content-filesgroup']").find('#uploadprogressbar .stop'))
		$(targetobj).parents("div[id^='app-content-filesgroup']").find('button.stop').fadeOut();
		$(targetobj).parents("div[id^='app-content-filesgroup']").find('#uploadprogressbar').fadeOut(function() {
			$(targetobj).trigger(new $.Event('resized'));
		});
	},

	_showProgressBarGroup: function(targetobj) {
		$(targetobj).parents("div[id^='app-content-filesgroup']").find('#uploadprogressbar').fadeIn();
		$(targetobj).trigger(new $.Event('resized'));
	},
	...
	init: function() {
		if ( $('#file_upload_start').exists() ) {
			var file_upload_param = {
				...
				add: function(e, data) {
					...
					if (freeSpace >= 0 && selection.totalBytes > freeSpace) {
						...
						data.errorThrown = t('files',
							'没有足够的可用空间, 正在上传 {size1} 但是可用空间为 {size2} ', {
				...
				submit: function(e, data) {
					...
					if ( ! data.formData ) {
						...
						data.formData = {
							...
							dir: data.targetDir || myFileList.getCurrentDirectory(),
				...
				fail: function(e, data) {
					...
					if (typeof data.textStatus !== 'undefined' && data.textStatus !== 'success' ) {
						if (data.textStatus === 'abort') {
							OC.Notification.show(t('files', '取消上传.'));
						} else {
				 ...
				 */
					...
					if (result.status === 'error' && result.data && result.data.message){
						...
						data.textStatus = 'servererror';
						data.errorThrown = t('files', '无法从服务器获取数据.');
						fu._trigger('fail', e, data);
						...
						if (data.files[0] === data.originalFiles[data.originalFiles.length - 1] &&
							result[0].directory === myFileList.getCurrentDirectory()) {
							// Scroll to the last uploaded file and highlight all of them
							//yosang
							var myfileList = _.pluck(data.originalFiles, 'name');
							myFileList.highlightFiles(myfileList);
						}
					}
				},
				/**
			...
			$("#app-content > div[id^='app-content-filesgroup'] #file_upload_start").each(function(index){
				if (supportAjaxUploadWithProgress()) {
					...
					// add progress handlers
					$(this).on('fileuploadstart', function(e, data) {
						OC.Upload.log('progress handle fileuploadstart', e, data);
						$(e.target).parents("div[id^='app-content-filesgroup']").find('#uploadprogressbar .stop').show();
						$(e.target).parents("div[id^='app-content-filesgroup']").find('#uploadprogressbar').progressbar({value: 0});
						OC.Upload._showProgressBarGroup(e.target);
					});
					...
					$(this).on('fileuploadprogressall', function(e, data) {
						...
						$(e.target).parents("div[id^='app-content-filesgroup']").find('#uploadprogressbar').progressbar('value', progress);
					});
					$(this).on('fileuploadstop', function(e, data) {
						OC.Upload.log('progress handle fileuploadstop', e, data);
						//yosang
						OC.Upload._hideProgressBarGroup(e.target);
					});
			...
			//yosangfileupload改成数组
			$("#app-content > div[id^='app-content-filesgroup'] #file_upload_start").each(function(index){
				var fileupload = $(this).fileupload(file_upload_param);
				window.file_upload_param = fileupload;
				...
				if (supportAjaxUploadWithProgress()) {
					...
					fileupload.on('fileuploadstart', function(e, data) {
						...
						$('#uploadprogresswrapper .stop').show();
						$('#uploadprogressbar').progressbar({value: 0});
						OC.Upload._showProgressBar(e.target);
					});
					fileupload.on('fileuploadprogress', function(e, data) {
						OC.Upload.log('progress handle fileuploadprogress', e, data);
						//TODO progressbar in row
		...
		$(window).on('beforeunload', function(e) {
			if (OC.Upload.isProcessing()) {
				return t('files', '文件正在上传. 若离开此页面 将会取消上传.');
			}
		});
		...
		$('#new li').click(function() {
			...
			var checkInput = function () {
				...
				} else if (myFileList.inList(filename)) {
			...
			form.submit(function(event) {
				...
				try {
					...
					if (myFileList.lastAction) {
						myFileList.lastAction();
					}
					var name = myFileList.getUniqueName(newname);
					switch(type) {
						case 'file':
							$.post(
								OC.filePath('files', 'ajax', 'newfile.php'),
								{
									dir: myFileList.getCurrentDirectory(),
									//dir: window.ytargetDir,
									filename: name
								},
								function(result) {
									if (result.status === 'success') {
										myFileList.add(result.data, {animate: true, scrollTo: true});
									} else {
										OC.dialogs.alert(result.data.message, t('core', '无法创建文件'));
									}
								}
							);
							break;
							...
							$.post(
								...
								function(result) {
									if (result.status === 'success') {
										...
										OC.dialogs.alert(result.data.message, t('core', '无法创建文件夹'));
									}
								}
							);
							break;
		...
		//yosang
		function setmyfilelist(){
			var groupdirarr = $("#app-navigation li[data-id^='filesgroup']").map(function(index,ele){
				var str = $(ele).data('id');
				//yosang
				return '/'+str.slice(10,str.length);
			})
			var myFileList ={};
			if(groupdirarr.length !== 0){
				groupdirarr.each(function(index,value){
					if(window.ytargetDir && window.ytargetDir.slice(0,2) == value){
						myFileList = GroupFileList;
						return false;
					}
					myFileList = FileList;
				})
		...
		//yosang
		// function setmyfileUpload(myFileList){
		// 	$fileupload = myFileList.$fileList.find('#file_upload_start');
		// 	$uploadprogressbar = myFileList.$fileList.find('#file_upload_start #uploadprogressbar');
		// 	return {$fileupload,$uploadprogressbar};
		// }
