	...
	BreadCrumb.prototype = {
		...
		render: function() {
			...
			for (var i = 0; i < parts.length; i++) {
				var part = parts[i];
				var $image;
				//为所有匹配的元素设置一个计算的属性值   以下attr函数将链接a的href值设为#
				var $link = $('<a></a>').attr('href', this.getCrumbUrl(part, i));
				$link.text(part.name);   //设置所有$link的文本内容
				$crumb = $('<div class="crumb svg"></div>');
				$crumb.append($link);  //向$crumb新增$link内容
				$crumb.attr('data-dir', part.dir);  //data-dir里存储完整路径
		 ...
		 */
		//此函数将路径分级存放在crumbs数组里，并返回
		_makeCrumbs: function(dir) {
			var crumbs = [];
			var pathToHere = '';
			// trim leading and trailing slashes 将路径最前和最后的/去掉
			dir = dir.replace(/^\/+|\/+$/g, '');
			var parts = dir.split('/');
			if (dir === '') {
				parts = [];
			...
			for (var i = 0; i < parts.length; i++) {
				...
				crumbs.push({
					dir: pathToHere,   //该变量存储完整路径
					name: part         //该变量存储新增子路径
				});
			}
			return crumbs;
		},
		...
		_resize: function() {
			...
			//yosang
            // $ellipsisCrumb = $('<div class="crumb ui-droppable svg"><span>...</span></div>');
