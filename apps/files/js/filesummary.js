...
(function() {
	...
	FileSummary.prototype = {
		...
		update: function() {
			...
			$dirInfo.html(n('files', '%n 文件夹', '%n 文件夹', this.summary.totalDirs));
			$fileInfo.html(n('files', '%n 文件', '%n 文件', this.summary.totalFiles));
			var fileSize = this.summary.sumIsPending ? t('files', '待计算') : OC.Util.humanFileSize(this.summary.totalSize);
			...
			if (this.summary.filter === '') {
				...
				$filterInfo.html(' ' + n('files', '匹配 \'{filter}\'', '匹配 \'{filter}\'', this.summary.totalDirs + this.summary.totalFiles, {filter: this.summary.filter}));
		...
		render: function() {
			...
			var directoryInfo = n('files', '%n 文件夹', '%n 文件夹', summary.totalDirs);
			var fileInfo = n('files', '%n 文件', '%n 文件', summary.totalFiles);
			var filterInfo = '';
			if (this.summary.filter !== '') {
				filterInfo = ' ' + n('files', '匹配 \'{filter}\'', '匹配 \'{filter}\'', summary.totalFiles + summary.totalDirs, {filter: summary.filter});
			}
			...
			if (!isNaN(summary.totalSize)) {
				fileSize = summary.sumIsPending ? t('files', '待计算') : OC.Util.humanFileSize(summary.totalSize);
				fileSize = '<td class="filesize">' + fileSize + '</td>';
			}
			var info = t('files', '{dirs} 和 {files}', infoVars, null, {'escape': false});
