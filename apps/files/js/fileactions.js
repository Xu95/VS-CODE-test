...
(function() {
	...
	FileActions.prototype = {
		...
		registerAction: function (action) {
			...
			if (_.isFunction(action.render) && (action.name!=='favorite')) {
		...
		_makeActionLink: function(actionSpec, context) {
			...
			var html = '<a href="#">';
			//yosang
			if (img && (img.indexOf('share')== -1)) {
				//console.log(img)
				html += '<img class="svg" alt="" src="' + img + '" />';
			}
			if (actionSpec.displayName && (actionSpec.displayName!=='共享') && (actionSpec.displayName!=='Favorite')) {
				//console.log('actionSpec.displayName'+actionSpec.displayName)
		...
		registerDefaultActions: function() {
			...
			// 	$.ajax({
			// 		url: 'http://121.49.107.4:55588/owncloud/index.php/apps/files/ajax/openapp.php',
			// 		data: {
			// 			filename:filename
			// 		},
			...
			// }, t('files', '打开'));
			this.register('file', 'Download', OC.PERMISSION_READ, function () {
                ...
                var re =/[\/]/g;
                console.log(context.fileList.getCurrentDirectory())

                console.log(context.fileList.getCurrentDirectory())
				var strdirectory = context.fileList.getCurrentDirectory()
                if(strdirectory.indexOf("personal") >= 0){
                    strdirectory = strdirectory.slice(9);
				}else{
                    strdirectory = strdirectory.slice(2);
				}
				console.log('strdirectory'+strdirectory);
                var newstrdirectory = strdirectory.replace(re, "\\")+"\\";
                //我的getcookie函数yosang
                ...
                var reg = /([^.]+)$/;
                var fileSuffix = filename.match(reg)[1];
				console.log(fileSuffix);
				console.log(strpanfu.slice(9,strlength-10)+':'+newstrdirectory+filename)
                $.post(
                    ...
                    function(res) {
                        ...
                        if(data.code == 800){
                            var radiohtml = '';
                            _.each(data.apps,function(element, index){
                                radiohtml+="<div class='appitem'><input name='openapp' type='radio' appindex='"+index+"' value='"+index+"'/>"+element.name+' (应用ID:'+element.id+")</input></div>";
                            })
                            layer.open({
                                title: [
                                    // '选择打开应用:'+window.FileList.getCurrentDirectory()+'/'+filename,
                                    '选择您想用来打开此文件的应用：',
                                    'background-color:#1d2d44; color:#fff;'
                                ],
                                content: "<div>"+radiohtml+"</div>"
                                ,btn: ['确定', '取消']
                                ,yes: function(index,layero){
                                    layer.close(index);
                                    // alert('window.selectappindex')
                                    console.log('window.selectappindex');
                                	console.log(window.selectappindex);
                                    setWinAapp();
                                    //获取userid
                                    ...
                                    OwnApp.openselectedApp(data.apps[window.selectappindex]);
                                }
                            });

                        }else if(data.code == 801){
                            layer.open({
                                content:'没有支持打开该文件的应用,请联系管理员订阅'
                            })
                        }else if(data.code == 802){
                            layer.open({
                                content:'请订阅'+data.name
                            })
                        }
