...
(function () {
	...
	Search.prototype = {
		...
		initialize: function($searchBox, $searchResults) {
			...
			this.clearwithoutblur = function() {
				// self.hideResults();
				// if(self.hasFilter(getCurrentApp())) {
				// 	self.getFilter(getCurrentApp())('');
				// }
			...
			this.changesearchBox = function() {
				console.log(window.commonFileList)
				console.log('window.commonFileList.searchValue'+window.commonFileList.searchValue)
				$searchBox.val(window.commonFileList.searchValue);
			}
			...
			$searchBox.keyup(function(event) {
				if (event.keyCode === 13) { //enter
					...
					window.commonFileList.searchValue = query;
					if (lastQuery !== query) {
						currentResult = -1;
						if (query.length > 2) {
							self.search(query);
